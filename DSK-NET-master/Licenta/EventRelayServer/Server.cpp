...
bool Server::listenForNewConnection()
{
    ...
    //here we accepted the connection
    //now we need to find out wether this is guest or admin (i.e has or has not priviledges)
    Packet PacketType=P_IdentifyRequest;
    std::string Message;
    sendPacketType(newConnection,PacketType);
    getString(newConnection,Message);
    if(std::strcmp(Message,"Guest")==0)
    {
    ...
    {
        std::cout<<"Admin Connected! Total number of admin connections"<<totalAdminConnections_+1<<std::endl;
        adminConnections_[totalAdminConnections_]=newConnection;
        CreateThread(NULL,NULL,(LPTHREAD_START_ROUTINE)adminHandlerThread,(LPVOID)(totalAdminConnections_),NULL,NULL);
        ++totalAdminConnections_;
        return true;
...
void Server::adminHandlerThread(int ID)
{
    std::string adminInput;
    while(true)
    {
        std::cout<<"Please provide the command you would like to execute. "<<std::endl;
        std::getline(std::cin, adminInput);
        if(!processAdminCommand(ID,adminInput))
        {
            std::cout<<"Unrecognized command! "<<std::endl;
        }
    }
    std::cout<<"Lost connection to client ID "<<ID<<std::endl;
    closesocket(serverPtr->adminConnections_[ID]);
    /*
	If we lost connection to a client, we close the socket we last used for his connection and delete him
	*/
	if(totalAdminConnections_>1)
    {
        if (ID == serverPtr->totalAdminConnections_) 
        {
		    serverPtr->totalAdminConnections_ = serverPtr->totalAdminConnections_ - 1;
	    }
	    else
        {
            for (int i = ID; i < serverPtr->totalAdminConnections_-1; i++) 
            {
                serverPtr->adminConnections_[i] = serverPtr->adminConnections_[i + 1];
            }
...
bool processAdminCommand(int ID, std::string Command)
{
    if(std::strcmp(Command,"CloseAll"==0)
    {
        //do that
        return TRUE;
    }
    if(std::strcmp(Command,"PrintConnections")==0)
    {
        //do that
        return TRUE
    }
